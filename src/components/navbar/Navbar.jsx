...
export const Navbar = ({ sitevisitmodal, setSiteVisitModal, setContactModal }) => {
  ...
  return (
      <div className="font-body fixed w-full z-20 top-0 start-0 bg-black ">
        ...
        <div className="max-w-7xl mx-auto px-5 lg:px-0 flex flex-wrap items-center justify-between py-[8px] z-40 ">
            ...
            <ul className="flex flex-col p-4 md:p-0 md:flex-row gap-12 w-full justify-between text-white">
              {navLinks.map((link, index) => (
                <li key={index}>
                  <a
                    href={link.href}
                    className="block border-b md:border-0 hover:border-gray-400 font-bold text-sm uppercase text-white hover:text-PrestigeBrown transition-all duration-300"
                  >
                    {link.name}
                  </a>
                </li>
          ...
          <div className="hidden lg:flex items-center">
            <a
              ...
              className="flex items-center bg-PrestigeGrey text-black text-bold px-4 py-2 rounded-lg shadow-lg hover:bg-opacity-90 transition"
            >
